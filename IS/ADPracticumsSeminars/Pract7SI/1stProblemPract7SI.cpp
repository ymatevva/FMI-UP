...
unsigned int increaseDecimal(unsigned int& decimal)
{
	return decimal += 1;
}
...
void increaseWithOne(const char from[], size_t fromSize, char to[], size_t toSize,unsigned int numeric)
{
	unsigned int decimal = fromRandomToDecimal(from, fromSize, numeric);
	increaseDecimal (decimal);
...
int main()
{
	...
	char from[MAX_SIZE] = { '0','1','1' };
	char to[MAX_SIZE];

	unsigned int numeric;
	std::cin >> numeric;

	increaseWithOne(from, MAX_SIZE, to,MAX_SIZE,numeric);
	print(to, MAX_SIZE);
	return 0;
}